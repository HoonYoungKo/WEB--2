...
<%
		...
		if (request.getParameter("submitID") != null) {
			submitID = Integer.parseInt(request.getParameter("submitID"));
		}
		if (submitID == 0){
			PrintWriter script = response.getWriter();
			script.println("<script>");
			script.println("alert('유효하지 않은 글입니다.')");
			script.println("location.href='managerbbs.jsp'");
			script.println("</script>");
		}
		
		Submit submit = new SubmitDAO().getSubmit(submitID);
	%>
	...
	<!-- Page Heading -->
      <h1 class="my-4" style="font-family: 'Gaegu', cursive;"><strong style="color:#6bbadb;">COWALLKER</strong>
        <small>ADM View</small>
      </h1>
		<div class="row">
				<table id="writeform" class="table table-striped" style="text-align:center; border: 1px solid #dddddd; font-family: 'Gaegu', cursive;" >
					<thead>
						<tr>
							<th colspan="3" style="background-color: #6bbadb; text-align:center;">신청글 보기</th>
						</tr>
					</thead>
					<tbody>
						<tr>
							<td style="width:20%;">신청작품</td>
							<td colspan="2"><%= submit.getSubmitTitle() %></td>
						</tr>
						<tr>
							<td>신청자</td>
							<td colspan="2"><%= submit.getUserID() %></td>
						</tr>
						<tr>
							<td>신청일자</td>
							<td colspan="2"><%=submit.getSubmitDate().substring(0 , 11) + submit.getSubmitDate().substring(11,13)+ "시" + submit.getSubmitDate().substring(14, 16) + "분" %></td>
						</tr>
						<tr>
							<td>인원수</td>
							<td colspan="2"><%= submit.getUserNumber() %></td>
						</tr>
						<tr>
							<td>핸드폰번호</td>
							<td colspan="2"><%= submit.getSubmitPhone() %></td>
						</tr>
						<tr>
							<td>질문사항</td>
							<td colspan="2" style="min-height:200px; text-align: left;"><%=submit.getSubmitContent().replaceAll(" ","&nbsp;").replaceAll("<","&lt;").replaceAll(">", "&gt;").replaceAll("\n", "<br>") %></td>
						</tr>
					</tbody>
				</table>
				<a href="managerbbs.jsp" class="btn btn-primary" style="font-family: 'Gaegu', cursive; margin-right:10px;">목록</a>
